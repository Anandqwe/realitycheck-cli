...
_VAGUE_PATTERNS: tuple[tuple[str, str, Severity], ...] = (
    (r"\bsole discretion\b", "sole discretion", Severity.HIGH),
    (r"\bwithout notice\b", "without notice", Severity.HIGH),
    (r"\bas deemed necessary\b", "as deemed necessary", Severity.MEDIUM),
    (r"\bat any time for any reason\b", "at any time for any reason", Severity.HIGH),
)

_ONE_SIDED_PATTERNS: tuple[tuple[str, str, Severity], ...] = (
    (r"\bmay terminate\b.{0,60}\bwithout notice\b", "termination without notice", Severity.HIGH),
...
def detect_missing_protections(clauses: list[Clause]) -> list[str]:
    ...
    if not re.search(payment_pattern, text):
        missing.append("payment_timeline")

    termination_notice_pattern = r"(terminate|termination).{0,60}(written notice|notice period|\d+\s+days)"
    if not re.search(termination_notice_pattern, text):
        missing.append("termination_notice")

    if not re.search(r"(cure period|opportunity to cure)", text):
        missing.append("cure_period")

    liability_cap_pattern = r"(liability).{0,80}(cap|limit|shall not exceed|maximum)"
    if not re.search(liability_cap_pattern, text):
        missing.append("liability_cap")

    breach_notification_pattern = r"(data breach|breach).{0,80}(notify|notification).{0,40}(hours|days)"
    if not re.search(breach_notification_pattern, text):
    ...
    if not re.search(ip_retained_pattern, text):
